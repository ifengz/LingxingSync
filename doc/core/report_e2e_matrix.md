...
| Overage Fee Charges | `GET_FBA_OVERAGE_FEE_CHARGES_DATA` | BLOCKED_UPSTREAM | audits 78、80 均 CANCELLED | 无 document、下载、SHA 或 raw；停止盲目重建 |
...
| Inventory Planning | `GET_FBA_INVENTORY_PLANNING_DATA` | PASS | audit 82，1,145 raw | raw-only；99 列真实合同已验证 |
...
- 当前 Runner 已接入：21 类。
- 已有历史完整非零生产 E2E：18/21。
- 未闭环：3 类，分别是 AFN Inventory by Country、Overage Fee Charges、Long-term Storage Fee Charges。
...
每次生产报告终态后立即更新本表：写明 audit、raw 行数、适用的对账结果，以及是否存在后续失败任务。不得只依据代码 allowlist、页面状态或一次创建成功更新为 PASS。
