...
- `GET status` 按 `type + account + store_id` 返回指定报表最近一次任务、`report_task_id/report_document_id/download_sha256` 证据及三类对账差异；旧调用省略 `type` 时只兼容 Customer Returns。临时下载 URL、签名凭证或 token 不得返回。
