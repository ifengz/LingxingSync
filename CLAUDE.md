...
6. **原始表结构化、与领星一致**：每个已验证的接口或正式报告合同各自对应一张 `ls_*` 原始表，列名 = 领星字段名，不翻译、不加工。对外数据表只能是代码注册、静态迁移、来源与粒度明确的固定数据产品；不做通用跨表拼接、动态 schema 或消费者页面适配。
...
3. **固定数据产品**：每张对外数据表都必须有静态 SQL 结构、来源 `ls_*` 表、关联方式、唯一键和粒度。当前 Listing 日维固定为 `store/channel/ASIN/SKU/business_date`；退货原因、订单配送地址等不同粒度域只可作为独立明细表发布。来源没有覆盖的值保持 `NULL`/未验证，禁止造零或猜关联。
4. **固定只读发布**：内部项目只能通过版本化 HTTPS `snapshot` / `changes` 数据集端点读取，使用每项目 token、dataset/store scope、keyset cursor 和分页。端点仅接受代码注册的数据表与字段 allowlist，不接受任意表名、路径或 SQL；消费者不得直连本库。`/dataset-fields` 只配置已注册数据表的字段与下游项目授权，永不执行 DDL。
