...
const FulfilledShipmentsReportType = "GET_AMAZON_FULFILLED_SHIPMENTS_DATA_GENERAL"

var fulfilledShipmentsHeader = []string{
...
type FulfilledShipment struct{ Values []string }

func ParseFulfilledShipments(downloaded []byte, compressionAlgorithm, contentType string) ([]FulfilledShipment, error) {
	return parseFixedReportRows(downloaded, compressionAlgorithm, contentType, "Amazon fulfilled shipments", fulfilledShipmentsHeader, func(values []string) FulfilledShipment {
		return FulfilledShipment{Values: values}
	})
}
