...
func (d *DBReportStore) SaveFBAStrandedInventory(ctx context.Context, id int64, rows []reportexport.FBAStrandedInventory, downloadSHA, documentID string) error {
	const insert = "INSERT INTO ls_fba_stranded_inventory\n" +
		...
		"VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)\n" +
		"ON DUPLICATE KEY UPDATE row_sha256=VALUES(row_sha256), `primary-action`=VALUES(`primary-action`), `date-stranded`=VALUES(`date-stranded`), `Date-to-take-auto-removal`=VALUES(`Date-to-take-auto-removal`), `status-primary`=VALUES(`status-primary`), `status-secondary`=VALUES(`status-secondary`), `error-message`=VALUES(`error-message`), `stranded-reason`=VALUES(`stranded-reason`), asin=VALUES(asin), sku=VALUES(sku), fnsku=VALUES(fnsku), `product-name`=VALUES(`product-name`), `condition`=VALUES(`condition`), `fulfilled-by`=VALUES(`fulfilled-by`), `fulfillable-qty`=VALUES(`fulfillable-qty`), `your-price`=VALUES(`your-price`), `unfulfillable-qty`=VALUES(`unfulfillable-qty`), `reserved-quantity`=VALUES(`reserved-quantity`), `inbound-shipped-qty`=VALUES(`inbound-shipped-qty`), `program`=VALUES(`program`)"
	values := make([][]string, len(rows))
	for i, row := range rows {
		values[i] = row.Values
	}
...
func (d *DBReportStore) SaveFBAEstimatedFees(ctx context.Context, id int64, rows []reportexport.FBAEstimatedFees, downloadSHA, documentID string) error {
	const insert = "INSERT INTO ls_fba_estimated_fees\n" +
		...
		"VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)\n" +
		"ON DUPLICATE KEY UPDATE row_sha256=VALUES(row_sha256), sku=VALUES(sku), fnsku=VALUES(fnsku), asin=VALUES(asin), `product-name`=VALUES(`product-name`), `product-group`=VALUES(`product-group`), brand=VALUES(brand), `fulfilled-by`=VALUES(`fulfilled-by`), `has-local-inventory`=VALUES(`has-local-inventory`), `your-price`=VALUES(`your-price`), `sales-price`=VALUES(`sales-price`), `longest-side`=VALUES(`longest-side`), `median-side`=VALUES(`median-side`), `shortest-side`=VALUES(`shortest-side`), `length-and-girth`=VALUES(`length-and-girth`), `unit-of-dimension`=VALUES(`unit-of-dimension`), `item-package-weight`=VALUES(`item-package-weight`), `unit-of-weight`=VALUES(`unit-of-weight`), `product-size-weight-band`=VALUES(`product-size-weight-band`), currency=VALUES(currency), `estimated-fee-total`=VALUES(`estimated-fee-total`), `estimated-referral-fee-per-unit`=VALUES(`estimated-referral-fee-per-unit`), `estimated-variable-closing-fee`=VALUES(`estimated-variable-closing-fee`), `expected-domestic-fulfilment-fee-per-unit`=VALUES(`expected-domestic-fulfilment-fee-per-unit`), `expected-efn-fulfilment-fee-per-unit-uk`=VALUES(`expected-efn-fulfilment-fee-per-unit-uk`), `expected-efn-fulfilment-fee-per-unit-de`=VALUES(`expected-efn-fulfilment-fee-per-unit-de`), `expected-efn-fulfilment-fee-per-unit-fr`=VALUES(`expected-efn-fulfilment-fee-per-unit-fr`), `expected-efn-fulfilment-fee-per-unit-it`=VALUES(`expected-efn-fulfilment-fee-per-unit-it`), `expected-efn-fulfilment-fee-per-unit-es`=VALUES(`expected-efn-fulfilment-fee-per-unit-es`), `expected-efn-fulfilment-fee-per-unit-se`=VALUES(`expected-efn-fulfilment-fee-per-unit-se`), `amazon-store`=VALUES(`amazon-store`), `product-size-tier`=VALUES(`product-size-tier`), `estimated-order-handling-fee-per-order`=VALUES(`estimated-order-handling-fee-per-order`), `estimated-pick-pack-fee-per-unit`=VALUES(`estimated-pick-pack-fee-per-unit`), `estimated-weight-handling-fee-per-unit`=VALUES(`estimated-weight-handling-fee-per-unit`), `expected-fulfillment-fee-per-unit`=VALUES(`expected-fulfillment-fee-per-unit`), `estimated-future-fee`=VALUES(`estimated-future-fee`), `estimated-future-order-handling-fee-per-order`=VALUES(`estimated-future-order-handling-fee-per-order`), `estimated-future-pick-pack-fee-per-unit`=VALUES(`estimated-future-pick-pack-fee-per-unit`), `estimated-future-weight-handling-fee-per-unit`=VALUES(`estimated-future-weight-handling-fee-per-unit`), `expected-future-fulfillment-fee-per-unit`=VALUES(`expected-future-fulfillment-fee-per-unit`)"
	values := make([][]string, len(rows))
	for i, row := range rows {
		values[i] = row.Values
	}
...
func (d *DBReportStore) SaveFBARemovalOrder(ctx context.Context, id int64, rows []reportexport.FBARemovalOrder, downloadSHA, documentID string) error {
	const insert = "INSERT INTO ls_fba_removal_order_details\n" +
		"(account_id, seller_id, store_id, report_task_id, `row_number`, row_sha256, `request-date`, `order-id`, `order-source`, `order-type`, `order-status`, `last-updated-date`, sku, fnsku, disposition, `requested-quantity`, `cancelled-quantity`, `disposed-quantity`, `shipped-quantity`, `in-process-quantity`, `removal-fee`, currency)\n" +
		"VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)\n" +
		"ON DUPLICATE KEY UPDATE row_sha256=VALUES(row_sha256), `request-date`=VALUES(`request-date`), `order-id`=VALUES(`order-id`), `order-source`=VALUES(`order-source`), `order-type`=VALUES(`order-type`), `order-status`=VALUES(`order-status`), `last-updated-date`=VALUES(`last-updated-date`), sku=VALUES(sku), fnsku=VALUES(fnsku), disposition=VALUES(disposition), `requested-quantity`=VALUES(`requested-quantity`), `cancelled-quantity`=VALUES(`cancelled-quantity`), `disposed-quantity`=VALUES(`disposed-quantity`), `shipped-quantity`=VALUES(`shipped-quantity`), `in-process-quantity`=VALUES(`in-process-quantity`), `removal-fee`=VALUES(`removal-fee`), currency=VALUES(currency)"
	values := make([][]string, len(rows))
	for i, row := range rows {
		values[i] = row.Values
	}
