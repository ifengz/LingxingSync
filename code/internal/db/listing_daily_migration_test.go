...
func TestListingDailyMigrationHasOneFactTableAndExplicitIdentityDimension(t *testing.T) {
	...
	for _, want := range []string{
		...
		"SESSIONS_DESKTOP", "SESSIONS_MOBILE", "SESSIONS_TOTAL", "CATE_RANK", "CATE_RANK_SOURCE", "SMALL_CATE_RANK", "SMALL_CATE_RANK_SOURCE", "REVIEW_COUNT", "RATING",
...
func TestListingDailyRankMigrationAddsExistingFactColumns(t *testing.T) {
	raw, err := os.ReadFile("../../migrations/075_add_listing_daily_rank_fields.sql")
	if err != nil {
		t.Fatalf("读取 listing 排名字段迁移失败: %v", err)
	}
	sql := strings.ToUpper(string(raw))
	for _, want := range []string{
		...
		if !strings.Contains(sql, want) {
			t.Fatalf("listing 排名字段迁移缺少 %q", want)
		}
	}
	for _, forbidden := range []string{"DROP TABLE", "DELETE FROM", "TRUNCATE"} {
		if strings.Contains(sql, forbidden) {
			t.Fatalf("listing 排名字段迁移不得使用破坏性语句 %s", forbidden)
		}
	}
}
