...
-- 两接口均 iterate_by_store=false（VC 账号级，返回行内不含 sid），故主键不含 sid：
--   实时销量：粒度 = (asin, startTime 小时窗)；
--   销量报表：粒度 = (asin, date 日)。
...
CREATE TABLE IF NOT EXISTS ls_vc_realtime_sales (
    ...
    PRIMARY KEY (account_id, asin, startTime),
    INDEX idx_asin (account_id, asin)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='VC 实时销量报表（小时粒度，账号级无 sid）';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='VC 销量报表（日粒度，账号级无 sid）';
